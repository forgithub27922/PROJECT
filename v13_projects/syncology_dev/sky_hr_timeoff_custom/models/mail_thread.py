    ...
    def _notify_record_by_email(self, message, recipients_data, msg_vals=False,
        ...
        for recipients_group_data in recipients_groups_data:
            ...
            for recipients_ids_chunk in split_every(recipients_max, recipients_ids):
                ...
                mail_list = []
                if create_values.get('mail_message_id'):
                    new_mail_message_id = self.env['mail.message'].browse(create_values.get('mail_message_id'))
                    if new_mail_message_id.id and new_mail_message_id.model == 'hr.leave':
                        leave = self.env[new_mail_message_id.model].browse(new_mail_message_id.res_id)
                        if leave and leave.state == 'confirm' and leave.holiday_status_id:
                            mail_list.append(leave.employee_id.work_email)
                            if leave.holiday_status_id.validation_type in ('hr', 'both'):
                                if leave.holiday_status_id.responsible_ids:
                                    email_ids = leave.holiday_status_id.responsible_ids.mapped('email')
                                    mail_list.extend(email_ids)
                            if leave.holiday_status_id.validation_type in ('manager', 'both'):
                                if leave.employee_id.parent_id:
                                    mail_list.append(leave.employee_id.parent_id.work_email)
                                if leave.employee_id.leave_manager_id:
                                    mail_list.append(leave.employee_id.leave_manager_id.email)

                create_values.update({'email_to': ','.join(mail_list)})
