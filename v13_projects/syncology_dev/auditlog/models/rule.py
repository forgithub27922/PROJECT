...
class AuditlogRule(models.Model):
    ...
    @api.model
    def action_create_model_rule(self):
        rule_dict = {}
        models = self.env['ir.model'].search([])
        rule_obj = self.env['auditlog.rule']
        for rule in rule_obj.search([]):
            rule_dict.update({rule.model_id.model: rule.id})

        for model in models:
            if model.model not in rule_dict and not model.transient:
                rule_obj.create({'name': model.name,
                                 'model_id': model.id,
                                 'log_type': 'full',
                                 'log_read': False,
                                 'log_write': True,
