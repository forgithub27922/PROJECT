...
class MagentoInvoiceListener(Component):
    ...
    _inherit = 'base.event.listener'
    _apply_on = ['account.move'] #odoo13change

    def on_invoice_paid(self, record):
        #return True # magento_stop
        if record.payment_mode_id and record.payment_mode_id.name =='payone_ratepay' and record.payment_mode_id.create_invoice_on == 'paid':
            self.invoice_create_bindings(record)

    def on_invoice_validated(self, record):
        #return True  # magento_stop
        if record.payment_mode_id and record.payment_mode_id.name =='payone_ratepay'  and record.payment_mode_id.create_invoice_on == 'open':
            self.invoice_create_bindings(record)
