...
class TestSaleOrder(MagentoSyncTestCase):
    ...
    def test_import_sale_order_with_configurable(self):
        ...
        prod1 = self.env['magento.product.product'].search(
            [('magento_id', '=', 512), ('backend_id', '=', self.backend.id)]
        )
        prod2 = self.env['magento.product.product'].search(
            [('magento_id', '=', 302), ('backend_id', '=', self.backend.id)]
        )
        ship = self.env['product.product'].search([('name', '=', 'ups_GND')])

        expected = [
            ExpectedOrderLine(
                product_id=prod1.openerp_id,
                name='Tori Tank',
                price_unit=60.,
                product_uom_qty=2.,
            ),
            ExpectedOrderLine(
                product_id=prod2.openerp_id,
    ...
    def test_import_sale_order_copy_quotation(self):
        ...
        self.assertEqual(binding.openerp_id, new)
    ...
    def test_import_sale_order_guest(self):
        ...
        self.assertEqual(partner_binding.magento_id, 'guestorder:145000008')
