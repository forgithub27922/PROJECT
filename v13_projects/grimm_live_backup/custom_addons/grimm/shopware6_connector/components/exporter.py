# -*- coding: utf-8 -*-
# © 2019 Dipak Suthar - Grimm Gastrobedarf.de
# © 2019 Partenics GmbH
...
from odoo.addons.connector.exception import (IDMissingInBackend,
                                             RetryableJobError)
from .backend_adapter import SHOPWARE_DATETIME_FORMAT
...
class Shopware6BaseExporter(AbstractComponent):
    """ Base exporter for Shopware """

    _name = 'shopware6.base.exporter'
    _inherit = ['base.exporter', 'base.shopware6.connector']
    _usage = 'record.exporter'

    def __init__(self, working_context):
        super(Shopware6BaseExporter, self).__init__(working_context)
        self.binding = None
    ...
    def _should_import(self):
        ...
        in Shopware and the last sync date in Odoo,
        if the former is more recent, schedule an import
        to not miss changes done in Shopware.
        """
        assert self.binding
        if not self.shopware6_id:
            return False
        sync = self.binding.sync_date
        if not sync:
            return True
        record = self.backend_adapter.read(self.shopware6_id,
    ...
    def run(self, binding, *args, **kwargs):
        ...
        self.shopware6_id = self.binder.to_external(self.binding)
        ...
        self.binder.bind(self.shopware6_id, self.binding)
        ...
        if self.binding:
            self.binding.updated_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            if not self.binding.created_at:
                self.binding.created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self._after_export()
        return result
...
class Shopware6Exporter(AbstractComponent):
    """ A common flow for the exports to Shopware """

    _name = 'shopware6.exporter'
    _inherit = 'shopware6.base.exporter'

    def __init__(self, working_context):
        super(Shopware6Exporter, self).__init__(working_context)
        self.binding = None

    def create_batch_json(self, binding, *args, **kwargs):
        """ Run the synchronization
    ...
    def _export_dependency(self, relation, binding_model,
                           ...
                           binding_field='shopware6_bind_ids',
    ...
    def _update(self, data):
        ...
        self.backend_adapter.write(self.shopware6_id, data)
    ...
    def _run(self, fields=None):
        ...
        if self.shopware6_id:
            ...
            if not record:
                return _('Nothing to export.')
            self.shopware6_id = self._create(record)
        return _('Record exported with ID %s on Shopware.') % self.shopware6_id
