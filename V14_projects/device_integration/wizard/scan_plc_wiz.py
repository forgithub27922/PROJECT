...
class ScanPLCWizard(models.TransientModel):
    ...
    def generate(self):
        ...
        path = self.env.company.lm_csv_path
        f1 = open(path + fields.Datetime.now().strftime('%d_%b_%Y_%H_%M_%S') + '.csv', 'w')
        csvw = csv.writer(f1)
...
class ScanPatternWizard(models.TransientModel):
    ...
    def on_barcode_scanned(self, barcode):
        ...
        if barcode in self.pattern_barcode_ids.mapped('pattern_barcode'):
            raise UserError("Barcode Already Scanned!")
        # Add a line in the Pattern Barcode O2M field
        self.write({'pattern_barcode_ids': [(0, 0, {'pattern_barcode': barcode})]})
