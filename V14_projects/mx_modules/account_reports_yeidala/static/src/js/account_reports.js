odoo.define('account_reports_yeidala.account_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        'value_changed': function(ev) {
             ...
             self.report_options.acc_partner_ids = ev.data.acc_partner_ids;
             ...
             return self.reload().then(function () {
                 self.$searchview_buttons.find('.account_acc_acc_filter').click();
    ...
    render_searchview_buttons: function() {
        ...
        // partner acc_acc
        /*
        if (this.report_options.acc_acc) {
            if (!this.M2MFilters) {
                var fields = {};
                ...
                if (!_.isEmpty(fields)) {
                    ...
                    this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_acc_acc_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_acc_acc_m2m').append(this.M2MFilters.$el);
            }
        }*/
        ...
        if (this.report_options.analytic) {
            if (!this.M2MFilters) {
                ...
                if (this.report_options.acc_acc) {
                    fields['account_ids'] = {
                        label: _t('Account Accounts'),
                        modelName: 'account.account',
                        value: this.report_options.account_ids.map(Number),
                    };
                }
                if (this.report_options.acc_partner) {
                    fields['acc_partner_ids'] = {
                        label: _t('Empresas'),
                        modelName: 'res.partner',
                        value: this.report_options.acc_partner_ids.map(Number),
                    };
                }
                if (!_.isEmpty(fields)) {
                    this.M2MFilters = new M2MFilters(this, fields);
