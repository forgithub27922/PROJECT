...
class wizard_bs_consolidate_report_wizard(models.TransientModel):
    _name ='wizard.bs.consolidate.report'
    ...
    def print_xls_report(self):
        """
            ...
            :return:
        """
        workbook = xlsxwriter.Workbook('balance_sheet_consolidated_report.xls')
        ...
        statement_merge_format = workbook.add_format({'bold':True,'align':'center','valign':'vcenter','font_size':10, \
                                                'num_format':'dd/mm/yy'})
        header_merge_format = workbook.add_format({'bold':True,'align':'center','valign':'vcenter', \
                                            'font_size':10,'border':1})
        main_group_data_format = workbook.add_format({'align':'left','valign':'vcenter','font_size':9,'bold':True})
        amount_main_group_data_format = workbook.add_format({'align':'right','valign':'vcenter','font_size':9,'bold':True})

        group_total_data_format = workbook.add_format({'align':'left','valign':'vcenter','font_size':9,'bold':True,'bg_color':'#808080'})
        amount_group_total_data_format = workbook.add_format({'align':'right','valign':'vcenter','font_size':9,'bold':True,'bg_color':'#808080'})
        ...
        name = "OFFICE AND PALACE OF HH DR.SHEIKH SULTAN BIN KHALIFA BIN ZAYED AL NAHYAN" + "\n"  \
                + "CONSOLIDATED BALANCE SHEET AS ON " + datetime.strptime(self.end_date,'%Y-%m-%d').strftime("%d-%B-%y")

        worksheet.merge_range(0, 0, 5, 2, "",bank_balance_merge_format)
        worksheet.merge_range(8, 0, 9, 13, name,bank_balance_report_merge_format)
        worksheet.merge_range(4, 7, 4, 9, "All Amount in AED",statement_merge_format)
        ...
        for main_grp in account_grp_obj:
            ...
            for child_group in child_ids:
                ...
                worksheet.write(self.account_row,0, child_group.name.rjust(len(child_group.name) + self.account_space),main_group_data_format)
                for company in company_ids:
                    amount = self.group_wise_bs_data(company,all_child_group_ids,account_account_type_obj)
                    set_column = self.account_dynamic_column_list[company.id]
                    amount = amount[0][0] if amount else 0.00
                    worksheet.write(self.account_row,set_column,formatLang(self.env, amount),amount_main_group_data_format)
                    child_group_total += amount
                worksheet.write(self.account_row,column,formatLang(self.env, child_group_total),amount_main_group_data_format)
                ...
                self.recursive_group(worksheet_data,company_ids,child_group,account_account_type_obj,main_grp)
            self.account_row+=1
            ...
            for company in company_ids:
                ...
                set_column = self.account_dynamic_column_list[company.id]
                amount = amount[0][0] if amount else 0.00
                bs_data[company].append(abs(amount))
                worksheet.write(self.account_row,set_column,formatLang(self.env, abs(amount)),amount_group_total_data_format)
                main_group_total += amount

            worksheet.write(self.account_row,column,formatLang(self.env, main_group_total),amount_group_total_data_format)
            self.account_row +=2

        worksheet.write(self.account_row,0,"Equity",group_total_data_format)
        equity_total_amount = 0.00
        for company in company_ids:
            equity_amount = self.get_equity_group_amount(company)
            set_column = self.account_dynamic_column_list[company.id]
            worksheet.write(self.account_row,set_column,formatLang(self.env, equity_amount),amount_group_total_data_format)
            equity_total_amount += equity_amount

        worksheet.write(self.account_row,column,formatLang(self.env, equity_total_amount),amount_group_total_data_format)
        ...
        self.write({
            'state': 'done',
            'file_download': base64.b64encode(open('balance_sheet_consolidated_report.xls','rb').read()),
            'name': 'balance_sheet_consolidated_report.xls'
        })
    ...
    def recursive_group(self,worksheet_data,company_ids,child_group,account_account_type_obj,main_grp):
        """
            ...
            :param bs_data: balance sheet dictionary
        ...
        for each in child_ids:
            ...
            for company in company_ids:
                group_amount = self.group_wise_bs_data(company,all_child_group_ids,account_account_type_obj)
                ...
                worksheet.write(self.account_row,group_company_set_column,formatLang(self.env, group_amount),amount_main_group_data_format)
            ...
            self.account_row +=1
            for company in company_ids:
                data = self.company_bs_data(company,each,account_account_type_obj)
                for record in data:
                    acocunt_id = account_obj.browse(record[0])
                    worksheet.write(self.account_row,0,acocunt_id.display_name.rjust(len(acocunt_id.display_name) + find_level * 2),account_data_format)
                    set_column = self.account_dynamic_column_list[company.id]
    ...
    @api.multi
    def recursive_account_wise_group(self,worksheet_data,company_ids,child_group,account_account_type_obj,main_grp,space):
        """
        ...
        for each in child_ids:
            ...
            self.group_row +=1
            self.recursive_account_wise_group(worksheet_data,company_ids,each,account_account_type_obj,main_grp,space)

    @api.multi
    def company_bs_data(self,company_id,each,account_account_type_obj):
        """
        ...
        SELECT
            ...
            aml.account_id,
            aml.company_id
        """ , (company_id.id,tuple(account_account_type_obj.ids),tuple(account_ids.ids),self.end_date))
        query_res = self._cr.fetchall()
        return query_res

    @api.multi
    def group_wise_bs_data(self,company_id,each,account_account_type_obj):
        """
            Get the Journal Item data based on given company,Account Group and Account type.
            :param company_ids: company recordset
            :param each: group recordset
        ...
        """ , (company_id.id,tuple(account_account_type_obj.ids),self.end_date,tuple(each.ids)))
    ...
    def account_grp_wise_xls_report(self,workbook):
        ...
        for company in company_ids:
            ...
            group_bs_data.setdefault(company,[])
            column +=1
        ...
        # equity_group = self.env.ref('sbk_group.account_group_main_6')
        equity_total_amount = 0.00
        for main_grp in account_grp_obj:
            main_group_total = 0.00
            ...
            for child_group in child_ids:
                all_child_group_ids = self.env['account.group'].search([('parent_id', 'child_of', child_group.id)])
                worksheet.write(self.group_row,0, child_group.name.rjust(len(child_group.name) + self.space),main_group_data_format)
                ...
                worksheet_data = {'worksheet':worksheet,'row':self.group_row,'column':column,
                                  'format':main_group_data_format,'workbook':workbook}
                self.recursive_account_wise_group(worksheet_data,company_ids,child_group,account_account_type_obj,main_grp,self.space)
            self.group_row+=1
            ...
            for company in company_ids:
                ...
                set_column = self.group_dynamic_column_list[company.id]
                amount = amount[0][0] if amount else 0.00
                group_bs_data[company].append(abs(amount))
                worksheet.write(self.group_row,set_column,formatLang(self.env, abs(amount)),amount_group_total_data_format)
                main_group_total += amount

            worksheet.write(self.group_row,column,formatLang(self.env, main_group_total),amount_group_total_data_format)

            self.group_row+=2

        worksheet.write(self.group_row,0,"Equity",group_total_data_format)
        for company in company_ids:
            equity_amount = self.get_equity_group_amount(company)
            set_column = self.group_dynamic_column_list[company.id]
            worksheet.write(self.group_row,set_column,formatLang(self.env, equity_amount),amount_group_total_data_format)
            equity_total_amount += equity_amount

        worksheet.write(self.group_row,column,formatLang(self.env, equity_total_amount),amount_group_total_data_format)
    ...
    def common_account_group_obj(self,company_ids,account_account_type_obj):
        ...
        self._cr.execute("""
                            ...
                            AND aml.date <= %s
                            AND aml.my_acc_group_id is not null
        """ , (tuple(company_ids.ids),tuple(account_account_type_obj.ids),self.end_date))
        query_res = self._cr.fetchall()
        final_grp_lst = []
        ...
        account_grp_obj -= self.env.ref('sbk_group.account_group_main_6')
        account_grp_obj -= self.env.ref('sbk_group.account_group_main_7')
        account_grp_obj -= self.env.ref('sbk_group.account_group_main_8')

        return account_grp_obj.sorted(key=lambda r: r.sequence)

    @api.multi
    def get_equity_group_amount(self,company):
        RETAINED_EARNINGS_AMOUNT = 0.00
        equity_line = self.env.ref('account_reports.account_financial_report_equity0').sudo()
        balance_sheet_financial_report = self.env.ref('account_reports.account_financial_report_balancesheet0').sudo()
        linesDict = {}
        currency_table = {}
        get_balance  = equity_line.sudo().with_context(date_to=self.end_date,state='posted',company_ids=company.ids).get_balance(linesDict,currency_table,balance_sheet_financial_report,['balance'])
        if get_balance:
            RETAINED_EARNINGS_AMOUNT = get_balance[0]['balance']
        return RETAINED_EARNINGS_AMOUNT
