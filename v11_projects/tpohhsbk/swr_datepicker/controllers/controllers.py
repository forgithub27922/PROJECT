...
class Extension(main.WebClient):
    ...
    def translations(self, mods=None, lang=None):
        ...
        if mods is None:
            mods = [x['name'] for x in request.env['ir.module.module']
                    .sudo().search_read(
        ...
        if langs:
            lang_params = langs.read([
                "name", "direction", "year_format", "month_format",
                "date_format", "time_format",
                "grouping", "decimal_point", "thousands_sep"])[0]

#        Regional languages (ll_CC) must inherit/override their \
#        parent lang (ll), but this is
#        done server-side when the language is loaded, \
#        so we only need to load the user's lang.
        ...
        return {
            ...
            'multi_lang': len(request.env['res.lang'].sudo().
                              get_installed()) > 1,
        }
